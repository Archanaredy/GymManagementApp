...
public class PrimaryMenuRecycleAdapter extends RecyclerView.Adapter<PrimaryMenuRecycleAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        Glide.with(holder.itemView.getContext()).load(menuOptionList.get(position).getImage()).placeholder(menuOptionList.get(position).getIcon()).error(menuOptionList.get(position).getIcon()).circleCrop().into(holder.icon);
